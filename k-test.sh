...
EOF
mpirun -np 12 pw.x <$name.$k.in> $name.$k.out & 
tail -f --pid=$(ps -elf |grep pw.x|grep -v grep |head -n 1 |awk '{print $4}') $name.$k.out
echo "kpoint=$k" >>ecutkp-energy
grep ! $name.$k.out >>ecutkp-energy
grep time $name.$k.out|tail -n 1 >>ecutkp-energy
done
